...
### 가설 3 
특정 와인의 balance를 정량적으로 계산하는 방법은 찾을 수 없었으나, 여기서는 당소, 산도의 값과 그 요소들의 평균까지의 거리가 balance를 나타낼 것이라는 가정 하에 회귀분석을 진행하였다. 여기서 balance를 구하는 방법은 크게 두 가지로 나누어 보았다.
...
따라서, 정규성을 만족하지 않고 등분산성이 완전히 확보되지 않았기 때문에 1번 가설에서는 회귀분석의 결과가 정확하지 않을 수 있다.
...
@@ -288,6 +308,16 @@ summary(lm_white_ori_step) 결과를 분석해 본다면, 우선 white에만 residual.sugar가 품질에 영향을 미친다는 것은 화이트 와인을 마시는 소비자가 상대적으로 당도를 더 중요하게 고려한다는 것으로 설명할 수 있을 것 같다. https://www.asiae.co.kr/article/2018060806513888770 에 의하면, 화이트와인은 레드와인과 달리 껍질이나 씨에서 색소나 탄닌을 수출하는 과정이 없다. 따라서 화이트와인에 대한 소비자의 요구 또한 신선하고, 너무 떫거나 쓰지 않는 것으로, 가볍고 산뜻한 와인을 좋아한다고 한다.
...
summary 결과 balance 값이 유의하지 않다고 나왔다. 정확한 원인은 알 수 없으나 나름대로 추청한다면 balance는 저런 간단한 식으로 구해지는 것이 아닐 수 있다. 출처로 제시한 사이트를 참고하면, 와인의 balance는 단순히 당도, 산도, 알코올 말고도 탄닌, 과일 풍미가 농축된 정도와 같은 여러 요인들에 영향을 받는다. 이러한 점이 가설검정을 할 때 사용된 balance를 구하는 데에는 고려되지 않았다. 또한 각 요소별로 가중치가 다르다거나, 더 복잡한 식으로 balance가 결정될 가능성 역시 존재한다. 이러한 점에서 우리가 구한 balance 수치가 와인의 quality를 결정하는 데 적절한 요인이 아닐 수 있다.
...
결과에 따르면 전체적인 정확도가 50%~60% 사이로 그렇게 높지 않은데, 이는 와인의 quality가 linear model을 따르지 않을 수 있으며, OLS를 최소화하는 방식으로 제작된 linear model이 Acuuracy를 최대로 높이는 문제의 의도와 차이가 있어서 발생하는 문제라고 생각된다. 또한, LM 모형은 분류 문제에 적용할 없는데 이 문제는 분류 문제이기에 한계가 있다.

따라서 우리는 이러한 문제를 해결하고 model의 Acuuracy를 높이고자 비선형적 모델인 랜덤포레스트 모형을 적용하여 linear model과의 성능차이를 분석하였다. 

### 5-2 랜덤포레스트 모형

```{r, message = FALSE,warning = FALSE}
rf<-df[ ,-1] #index부분 제거
split = sample.split(rf$quality, SplitRatio = 0.8) #train과 test set을 80:20의 비율로 나누었다.
train = subset(rf, split == TRUE)
test = subset(rf, split == FALSE)

set.seed(5497)
fit <- randomForest(as.factor(quality) ~ .,data=train)

prediction <- predict(fit, test[,-which(names(test)=="quality")])
table(test$quality, prediction) #Confusion matrix
error <- mean(test$quality != prediction)
paste('Accuracy',round(1-error,4))  
```

Accuracy는 약 0.68정도로 linear 모델보다 정확도가 올라갔음을 확인할 수 있었다. 


## 6) 각 조원이 보고서 및 발표에 기여한 바
### 전재현 : 1번 가설 검정, 최종보고서 정리
### 조용수 : 3번 가설 검정, 탐색적 자료 분석 부분 작성, 발표
### 박지우 : 발표
